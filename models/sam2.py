...
class SAM2Image:
    ...
    def add_point(
        self, point_coords: tuple[int, int], is_positive: bool, label_id: int
    ) -> dict[int, np.ndarray]:
        ...
        if label_id not in self.point_coords:
            ...
            self.point_coords[label_id] = np.append(
                self.point_coords[label_id], np.array([point_coords]), axis=0
            )
            self.point_labels[label_id] = np.append(
                self.point_labels[label_id], 1 if is_positive else 0
            )

        return self.decode_mask(label_id)

    def set_box(
        self, box_coords: tuple[tuple[int, int], tuple[int, int]], label_id: int
    ) -> dict[int, np.ndarray]:
    ...
    def decode_mask(self, label_id: int) -> dict[int, np.ndarray]:
        ...
        if concat_coords.size == 0:
            mask = np.zeros(
                (self.orig_im_size[0], self.orig_im_size[1]), dtype=np.uint8
            )
        else:
            mask, _ = decoder(
                image_embed,
...
class SAM2ImageEncoder:
    def __init__(self, path: str) -> None:
        ...
        self.session = ort.InferenceSession(
            path, so, providers=ort.get_available_providers()
        )
